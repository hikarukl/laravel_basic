...
@section('main')
    ...
    <div class="max-w-7xl mx-auto mt-3 pl-5 pr-5 pt-16 md:pt-0 md:grid md:grid-cols-3">
        <div class="col-span-2 md:col-span-2">
            <div class="grid grid-cols-1 auto-cols-max md:text-left md:flex md:flex-wrap md:justify-center">
                ...
                @foreach($post['content'] as $content)
                    @if(is_string($content) && !preg_match("/.*(Ảnh:).*/", $content))
                        <p class="text-lg text-gray-700 mb-4">
                            {!! $content !!}
                        </p>
                    @else
                        @if(is_array($content) && $content['type'] == 'img')
